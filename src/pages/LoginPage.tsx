    ...
    <section className="w-screen h-screen flex">
      <div className="w-full flex-col gap-4 bg-muted/60 flex items-center justify-center h-full">
        ...
        <div className="text-center">
          <p className="text-2xl font-bold text-muted-foreground/90">Notetools Inc</p>
        <p className="text-muted-foreground/80 font-light text-base">Gestão inteligente</p>
        </div>
      </div>
      <div className="bg-white w-full h-screen space-y-10 flex flex-col items-center justify-center">
        <div className="flex flex-col justify-center items-center">
          <h1 className="text-xl font-semibold text-muted-foreground/90">Login</h1>
          <small className="text-xs text-muted-foreground/70">Faça login para continuar</small>
        </div>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="group relative">
                  <FormLabel className="bg-white absolute -translate-y-2 translate-x-2 pr-1">Username</FormLabel>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="group relative">
                    <FormLabel className="bg-white absolute -translate-y-2 translate-x-2 pr-1">Senha</FormLabel>
            ...
            <Button disabled={isLoading} className="w-full">
              {isLoading ? <Loader className="w-3 h-3 spin-in repeat-infinite text-background"/> :"Entrar"}
            </Button>
          </form>
        </Form>
        <div className="flex flex-col justify-center items-center gap-2">
          <img src="/src/assets/icon.svg" className="w-5" />
          <small className="text-sm font-medium text-muted-foreground/80">Notetools</small>
